...
import { Plus, ArrowLeft, Wind } from 'lucide-react';
...
const CoolingProcessMonitoringPage = () => {
  ...
  const fetchLogs = useCallback(async () => {
    try {
      setLoading(true);
      const res = await axios.get('/api/cooling-process-logs');
      setLogs(res.data || []);
    } catch (err) {
      console.error('Failed to fetch cooling process logs', err);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const filteredLogs = useMemo(() => {
    return logs.filter(log => {
      ...
      const matchMethod = log.cooling_method?.toLowerCase().includes(q);
      const matchLoc = log.storage_location?.toLowerCase().includes(q);
      const matchDate = log.log_date?.includes(q) || log.start_date?.includes(q);
      return matchFood || matchStaff || matchMethod || matchLoc || matchDate;
    });
  }, [logs, searchQuery]);

  return (
    <PageLayout>
      <Head title="Cooling Process Logs" />
      ...
      <div>
        ...
        <div className="panel-header-row">
          <div>
            <h1 className="page-title">Cooling Process Logs</h1>
            <p className="page-subtitle" style={{ color: 'var(--color-text-secondary)', marginTop: '4px' }}>
              CCP-6 Monitoring — Natural cooling compliance (cool to ≤8°C within 2 hours).
            </p>
          </div>
          <Button variant="primary" icon={Plus} onClick={() => router.visit('/haccp-logs/cooling-process/add')}>
            Add Entry
          </Button>
        </div>
        ...
        <Card style={{ padding: 0, overflow: 'hidden' }}>
          ...
          {loading ? (
            ...
            <div style={{ padding: '60px', textAlign: 'center', color: 'var(--color-text-secondary)' }}>
              No cooling process logs found. Click "Add Entry" to record one.
            </div>
...
export default CoolingProcessMonitoringPage;
