...
const PestControlMonitoringPage = () => {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Are you sure you want to delete this pest control log entry?')) return;
    try {
      await axios.delete(`/api/pest-control-logs/${id}`);
      fetchLogs();
    } catch (err) {
              ...
              <tbody>
                {filteredLogs.map(log => (
                  <tr key={log.id}>
                    ...
                    <td>
                      <strong style={{ color: 'var(--color-text-primary)' }}>{log.check_type}</strong>
                    </td>
                    <td>
                      {log.status === 'Passed' ? (
